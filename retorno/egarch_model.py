...
if __name__ == "__main__":
    ...
    p = 1
    q = 1
    modelo = arch_model(retornos, vol='EGARCH', p=p, q=q, dist='skewstudent', rescale=False)
    resultado = modelo.fit(disp='off')
    print("\nResumo do ajuste do modelo EGARCH(1,1):")
    ...
    residuos.to_csv('residuos_egarch.csv', header=True)
    volatilidade.to_csv('volatilidade_egarch.csv', header=True)
    ...
    plt.plot(volatilidade, label='Volatilidade Condicional (EGARCH)')
    plt.title('Volatilidade Estimada ao Longo do Tempo - EGARCH')
    plt.xlabel('Data')
    plt.ylabel('Volatilidade')
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'volatilidade_egarch.png'))
    plt.show()

    # Gráfico dos resíduos
    plt.figure(figsize=(12, 5))
    plt.plot(residuos, label='Resíduos (e_hat)', color='orange')
    plt.title('Resíduos do Modelo EGARCH')
    plt.xlabel('Data')
    plt.ylabel('Resíduo')
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'residuos_egarch.png'))
    plt.show()

    # Gráfico dos resíduos ao quadrado
    plt.figure(figsize=(12, 5))
    plt.plot(residuos**2, label='Resíduos ao Quadrado (e_hat²)', color='green')
    plt.title('Resíduos ao Quadrado do Modelo EGARCH')
    plt.xlabel('Data')
    plt.ylabel('Resíduo ao Quadrado')
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'residuos2_egarch.png'))
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'acf_residuos_egarch.png'))
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'acf_residuos2_egarch.png'))
    plt.show()
    ...
    lags_to_test = [1, 3*(p+q)-1, 5*(p+q)-1]
